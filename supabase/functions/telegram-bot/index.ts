...
// ── Foreman Photo FSM ───────────────────────────────────────
// Flow: SELECT_TYPE → SELECT_FACADE → SELECT_FLOOR → UPLOAD (up to 5) → COMMENT → CONFIRM → SENT
const PHOTO_TYPES: Record<string, string> = { daily: "Ежедневный", brackets: "Кронштейны", frame: "Каркас", glass: "Заполнение" };

async function startPhotoFSM(chatId: number, user: BotUser, session: any, photoType: string) {
  const projectId = session?.context?.project_id;
...
async function screenPhotoFloor(chatId: number, user: BotUser, session: any, facadeId: string) {
...
async function screenPhotoUpload(chatId: number, user: BotUser, session: any, floorId: string) {
...
async function handlePhotoFile(chatId: number, user: BotUser, session: any, fileUrl: string) {
  const urls = [...(session.context.photo_urls || []), fileUrl];
  const ctx = { ...session.context, photo_urls: urls };
  const count = urls.length;
  if (count >= 5) {
    // Max reached, proceed to comment
    await tgEdit(chatId, session.message_id,
...
async function screenPhotoComment(chatId: number, user: BotUser, session: any) {
  const ctx = session.context;
...
async function handlePhotoComment(chatId: number, user: BotUser, session: any, comment: string | null) {
...
async function handlePhotoConfirm(chatId: number, user: BotUser, session: any) {
  const ctx = session.context;
  // Save to bot_documents
  await db.from("bot_documents").insert({
    project_id: ctx.project_id, sender_id: user.user_id,
    doc_type: `photo_${ctx.photo_type}`, file_url: (ctx.photo_urls || [])[0] || null,
    comment: ctx.photo_comment || null, recipients: ["pm", "pto"],
    status: "sent",
  });
  // Create inbox for PM
  await db.from("bot_inbox").insert({
    project_id: ctx.project_id, from_user_id: user.user_id,
    from_role: "foreman", to_roles: ["pm", "pto"],
    type: "photo_report", title: `📸 ${ctx.photo_label} · ${ctx.facade_name} эт.${ctx.floor_number}`,
    description: ctx.photo_comment || null, file_url: (ctx.photo_urls || [])[0] || null,
    status: "new",
  });
  // Update floor photos
...
async function handleUpdate(update: any) {
  ...
  if (update.message) {
    ...
    if (user && session && session.state !== "IDLE") {
      ...
      if (session.state === "PHOTO_COMMENT") {
        return handlePhotoComment(chatId, user, session, text.trim());
      }
    }
  }
